...
export class PaymentService {
  /**
   * Creates a new Razorpay order or retrieves an existing active one for user checkout.
   * Leverages Prisma transactions and "SELECT FOR UPDATE" row locking to prevent duplicate concurrent payments.
   */
  async createRazorpayOrder(userId: string, orderId: string): Promise<any> {
    try {
      ...
      return await prisma.$transaction(async (tx) => {
        // Step 1: Lock the order row using SELECT FOR UPDATE to prevent race conditions
        ...
        // Custom orders are allowed partial payments (deposit vs balance), standard orders must not be already paid
        ...
        // Step 2: Calculate successful payments already paid towards this order
        ...
        // Prevent extra payments if order is already fully paid
        if (totalPaid >= order.totalAmount) {
          throw new AppError(400, 'Order is already fully paid');
        }

        // Check if there is an existing pending payment record for this session
        ...
        // Step 3: Determine payment amount. Custom orders require a 20% advance payment first, then the remaining 80% later.
        isCustomOrder;
        let payAmount = order.totalAmount;
        if (isCustomOrder) {
          if (successfulPayments.length === 0) {
            payAmount = Math.round(order.totalAmount * 0.20); // 20% advance
          } else {
            payAmount = Math.round(order.totalAmount - totalPaid); // Remaining balance
          }
        }

        // Setup Razorpay order options. Note: Razorpay expects amount in paise (cents)
        const options = {
        ...
        // Create order via Razorpay API SDK
        const rzpOrder = await razorpay.orders.create(options);

        // Step 4: Store a local pending payment record in our database
  ...
  /**
   * Verifies Razorpay payment signature submitted by the frontend client.
   * Updates local payment status, marks orders paid or pending, and triggers notifications.
   */
  async verifyPayment(
    userId: string,
    orderId: string,
    ...
    // Step 1: Verify the HMAC SHA256 signature generated by Razorpay
    ...
    // Step 2: Update status to success if not already paid
    if (order.paymentStatus !== PaymentStatus.PAID) {
      ...
      // Compute total paid amounts for this order
      ...
      if (isCustom && order.status !== OrderStatus.PENDING) {
        // If Custom request already processed but we are verifying next payment step
      ...
      // Determine if fully paid or partially paid (for custom orders)
      ...
      // If it was first payment (advance), confirm the order from PENDING status
      ...
      // Step 3: Handle automatic quotation acceptance for custom request orders
      try {
      ...
      // Step 4: Dispatch email notification to customer
  ...
  /**
   * Processes incoming asynchronous webhook notifications sent directly from Razorpay.
   * This handles offline or background state updates, like success/failure of payments.
   */
  async handleWebhook(rawBody: string, signature: string): Promise<void> {
    // Step 1: Securely verify webhook authenticity using the secret signature header
    ...
    // Step 2: Idempotency check. Make sure we don't process duplicate events.
    if (eventId) {
      try {
    ...
    // Step 3: Handle payment.captured (successful payment)
    if (event === 'payment.captured') {
      ...
      await prisma.$transaction(async (tx) => {
        ...
        // Update database order's payment status & system order status (CONFIRMED if it was PENDING)
        ...
        // Record event in OrderStatusHistory log
    ...
    // Step 4: Handle payment.failed (failed payment event)
